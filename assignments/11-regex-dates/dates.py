...
def main():
    ...
    monthsss = ("january,february,march,april,may,june,july,august,september,october,november,december")
    months = list(monthsss.split(','))
    #print(months)
    months2 = ['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec']
    #print(date_fh)
    #for dates in date_fh:
    dates = date_fh
    ...
    if match2:
        notany = True
        if match2.group(3)!=None and len(match2.group(3))<2:
            redo = str(0)+str(match.group(3))
            formater2= '{}-{}-{}'.format(match2.group(2)if len(match2.group(2))==4 else str(20)+match2.group(2), match2.group(1)if len(match2.group(1))==2 else str(0)+match2.group(1),'01' if match2.group(3)==None else redo)
            print(formater2)
        else:
